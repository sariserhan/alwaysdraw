...
export const HELP_TRANSLATIONS: Record<Locale, HelpTranslation> = {
  en: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Fill Bucket, Symmetry & Stickers",
      creativeToolsDesc: "Tap to flood fill enclosed areas (F). Enable 2-way, 4-way, or 8-way mandala symmetry (🔮). Stamp retro sprites/emojis, spray stencils (T), measure distances (R), or shine laser trails (L).",
      ...
      shieldDesc: "Admin Protected Zones (Mural Shield) lock designated artwork from overwriting. Admin panel (Shift+A) enables image stamping, area wipe, and client rollback.",
    },
  ...
  fr: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Remplissage, Symétrie & Autocollants",
      creativeToolsDesc: "Remplissage par pot de peinture (F). Symétrie mandala à 2, 4 ou 8 axes (🔮). Tamponnez des stickers rétro, meurez avec la règle (R), pochoirs (T) ou laser (L).",
      ...
      shieldDesc: "Zones protégées (Mural Shield) verrouillant les œuvres. Panneau d'administration (Shift+A) pour le tampon d'images et l'annulation.",
    },
  ...
  ar: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 التعبئة، التناظر والملصقات",
      creativeToolsDesc: "تعبئة سريعة (F). فعال التناظر المندالا لـ 2 و 4 و 8 محاور (🔮). ضع ملصقات ريترو، استخدم المسطرة (R)، الاستنسل (T) والليزر (L).",
      ...
      shieldDesc: "مناطق محمية لحظر التعديل على الأعمال الفنية. لوحة الإدارة (Shift+A) لختم الصور والتراجع.",
    },
  ...
  ru: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Заливка, Симметрия и Стикеры",
      creativeToolsDesc: "Быстрая заливка областей (F). Симметрия мандалы на 2, 4 или 8 осей (🔮). Стикеры, линейка (R), трафареты (T) и лазерный след (L).",
      ...
      shieldDesc: "Защищенные зоны (Mural Shield) блокируют перезапись рисунков. Панель админа (Shift+A) для штамповки изображений и отката.",
    },
  ...
  es: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Relleno, Simetría y Pegatinas",
      creativeToolsDesc: "Relleno de pintura (F). Simetría mandala de 2, 4 u 8 ejes (🔮). Pega pegatinas retro, mide con la regla (R), plantillas (T) y láser (L).",
      ...
      shieldDesc: "Zonas protegidas (Mural Shield) para bloquear sobreescrituras. Panel de administración (Shift+A) para estampar imágenes y revertir.",
    },
  ...
  pt: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Preenchimento, Simetria e Autocolantes",
      creativeToolsDesc: "Preenchimento por balde (F). Simetria mandala de 2, 4 ou 8 eixos (🔮). Cole autocolantes retro, meça com a régua (R), moldes (T) e laser (L).",
      ...
      shieldDesc: "Zonas protegidas (Mural Shield) para bloquear edições. Painel de administração (Shift+A) para carimbar imagens e reverter.",
    },
  ...
  tr: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 Boya Kovası, Simetri & Çıkartmalar",
      creativeToolsDesc: "Kapalı alanları tek tıkla doldurun (F). 2'li, 4'lü veya 8'li mandala simetrisini açın (🔮). Retro çıkartmalar yapıştırın, cetvelle ölçün (R), şablon (T) ve lazer (L) kullanın.",
      ...
      shieldDesc: "Korumalı Alanlar (Mural Shield) sanat eserlerinin üzerine çizilmesini engeller. Yönetici paneli (Shift+A) ile resim aktarma ve geri alma.",
    },
  ...
  ja: {
    ...
    categories: {
      ...
      creativeToolsTitle: "🪄 バケツ塗りつぶし・対称描画・ステッカー",
      creativeToolsDesc: "ワンタップで領域を塗りつぶし（F）。2方向・4方向・8方向のマンダラ対称描画（🔮）。レトロステッカー、定規（R）、ステンシル（T）、レーザー（L）。",
      ...
      shieldDesc: "保護エリア（Mural Shield）で作品の上書きを防止。管理者パネル（Shift+A）で画像スタンプやロールバックを実行。",
    },
